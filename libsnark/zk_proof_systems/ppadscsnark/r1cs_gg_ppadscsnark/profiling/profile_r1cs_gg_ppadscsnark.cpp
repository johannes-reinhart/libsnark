/** @file
 *****************************************************************************
 Profiling program that exercises the ppADSCSNARK (first authenticate,
...
#define APPLICATION_NAME "ADSC-SNARK profiling application"

using namespace libsnark;
typedef default_r1cs_gg_ppadscsnark_pp PP;
...
adscsnark_profile_t profile_r1cs_gg_ppadscsnark(
    ...
    r1cs_gg_ppadscsnark_constraint_system<PP> constraint_system = r1cs_to_r1cs_adsc(std::move(example.constraint_system), private_input_size, state_size);
    ...
    r1cs_gg_ppadscsnark_keypair<PP> keypair = r1cs_gg_ppadscsnark_generator<PP>(constraint_system, example.state_assignment[0]);
    tend = libff::get_nsec_cpu_time();
    profile_result.generator_runtime = tend - tstart;

    tstart = libff::get_nsec_cpu_time();
    r1cs_gg_ppadscsnark_processed_verification_key<PP> pvk = r1cs_gg_ppadscsnark_verifier_process_vk<PP>(keypair.vk);
    tend = libff::get_nsec_cpu_time();
    profile_result.verifier_preprocessing_runtime = tend - tstart;

    r1cs_gg_ppadscsnark_proof<PP> previous_proof = keypair.initial_proof;
    for(size_t t = 0; t < samples; ++t) {
        tstart = libff::get_nsec_cpu_time();
        authentication_tags<PP> tags = r1cs_gg_ppadscsnark_authenticate(keypair.aks[0], constraint_system.primary_input_size + 1, t, example.private_input[t]);
        tend = libff::get_nsec_cpu_time();
        ...
        tstart = libff::get_nsec_cpu_time();
        r1cs_gg_ppadscsnark_proof<PP> proof = r1cs_gg_ppadscsnark_prover<PP>(keypair.pk,
                                                                               constraint_system,
        ...
        tstart = libff::get_nsec_cpu_time();
        bool verified = r1cs_gg_ppadscsnark_online_verifier_strong_IC<PP>(pvk, example.primary_input[t], proof, previous_proof, t);
        tend = libff::get_nsec_cpu_time();
...
int main(int argc, const char * argv[])
{
    ...
    if (vm.count("profile")){
        ...
        std::cout << "inputs 2^x, states 2^x, constraints, auth (s), prove (s), ver (s)" << std::endl;
        ...
        private_inputs_num += private_inputs_range[2]){
            for(int state_num = state_range[0];
                ...
                profile = profile_r1cs_gg_ppadscsnark(pow2(num_constraints),
                                                      pow2(public_io_size),
